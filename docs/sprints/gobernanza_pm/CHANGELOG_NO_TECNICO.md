...
## 2026-06-22 � Correcci�n del Selector de Formularios en el Dise�ador de Procesos BPMN
**Autor**: Agente Frontend (?? FRONTEND - VUE3)
**�Qu� es?**: Se corrigi� el selector de formularios en el dise�ador de procesos BPMN para que muestre los formularios reales creados por el usuario, eliminando datos de prueba que aparec�an anteriormente.
**�Para qu� sirve?**: Para asegurar que al configurar una tarea en un proceso, los usuarios solo puedan seleccionar formularios que realmente existen y est�n listos para usarse, evitando confusiones y errores con datos falsos.
**�De d�nde viene?**: Historia de Usuario US-005, Criterios de Aceptaci�n 39 y 40.
**�Qu� deber�a hacer?**:
- El men� desplegable de formularios muestra �nicamente los formularios reales.
- Si no hay formularios, muestra la lista vac�a en vez de informaci�n inventada (mocks).

**Estado**: ? Listo

---

## 2026-06-22 - Estabilización de Pruebas Automatizadas y Teardown de Contenedores
**Autor**: Agente Frontend (👨‍💻 FRONTEND - VUE3) — Rama DevDavid
**¿Qué es?**: Se configuró la limpieza automática y total de la infraestructura temporal (contenedores) usada durante la certificación E2E. Además, se dotó al sistema de pruebas de un tiempo prudente para el "arranque en frío" visual.
**¿Para qué sirve?**: Para garantizar que nuestra infraestructura de pruebas no deje "basura" en los servidores que bloquee puertos, y sea 100% confiable, erradicando falsas alarmas provocadas por retrasos normales de compilación.
**¿De dónde viene?**: Handoff Arquitectónico: Fix Playwright y Cierre CA-39/CA-40.
**¿Qué debería hacer?**:
- Al terminar, se destruye todo ambiente temporal de pruebas sin dejar contenedores fantasma.
- Las pruebas esperan pacientemente a que la interfaz esté lista.

**Estado**: ✅ Listo

---

## 2026-06-23 - Resoluci�n Definitiva de Flaky Tests en Modelador BPMN
**Autor**: Agente Frontend ????? FRONTEND - VUE3 | Rama DevDavid
**�Qu� es?**: Se corrigi� un error sutil en la prueba automatizada que provocaba fallos falsos al interactuar demasiado r�pido con el modal de bienvenida antes de que la pantalla cargara por completo.
**�Para qu� sirve?**: Para que el robot de pruebas no intente hacer clic 'a ciegas' mientras la aplicaci�n a�n se est� dibujando en la pantalla, asegurando una validaci�n 100% certera de la funcionalidad.
**�De d�nde viene?**: An�lisis profundo de la reactividad de Vue 3 en escenarios de 'Cold Start' (Arranque en fr�o).
**�Qu� deber�a hacer?**: 
...
## 2026-06-24 - Correcci�n de Cat�logo de Formularios y Enrutamiento en Dise�ador BPMN
**Autor**: Agente Backend (?? BACKEND - JAVA) / Agente Frontend (?? FRONTEND - VUE3) - Rama DevDavid
**�Qu� es?**: Se eliminaron datos simulados (mocks) en el cat�logo de formularios para mostrar informaci�n real de la base de datos. Adicionalmente, se corrigieron enlaces rotos y rutas de navegaci�n dentro del dise�ador de procesos BPMN que llevaban a p�ginas no encontradas (error 404).
**�Para qu� sirve?**: Para que al dise�ar un proceso y asignar formularios a las tareas, el sistema liste los formularios verdaderos creados por la organizaci�n. Tambi�n asegura que los usuarios puedan navegar fluidamente dentro del �rea de modelado sin toparse con pantallas de error al acceder a subprocesos o al dise�ador principal.
**�De d�nde viene?**: Resoluci�n de bugs cr�ticos detectados durante la Certificaci�n UAT Manual (Sprint PM-01). Reporte de Agente QA (BUG-J02-001, BUG-J02-002, BUG-J02-003).
**�Qu� deber�a hacer?**: 
- El men� de selecci�n de formularios en el dise�ador muestra datos reales de PostgreSQL.
- Ingresar a la secci�n de modelador no resulta en un error 404, redirigiendo correctamente a la herramienta.
