# 🔧→🎨 Handoff Bug-Fix: BUG-FIX LEAD → FRONTEND - VUE
...
**Dependencia:** Ninguna (o indicar handoff de Backend si aplica)
...
| # | Criterio | Evidencia |
|---|----------|-----------|
| 1 | El bug reportado ya no se reproduce | [COMPLETAR — Acción de verificación] |
| 2 | No se introdujeron regresiones en el build | `npm run build` → `✓ built in Xs` |
| 3 | Código documentado con @Traceability | `grep "@Traceability.*BUG-FIX" [archivo]` → 1+ resultado |
| 4 | Contrato API respetado | Endpoints consumidos coinciden con el backend real |
...
5. **Build obligatorio:** Ejecuta el protocolo Zero-Trust UI documentado en `.agents/skills/frontend_build_audit/SKILL.md`.
...
4. cat docs/architecture/adr-002-vue3-microfrontends.md
5. cat .agentic-sync/bug_diagnosis_[ID].md
6. cat .agentic-sync/handoff_bugfix_frontend.md
...
- PROHIBIDO usar Options API. Solo Composition API (<script setup>).
...
3. Guarda tu solicitud de revisión en `.agentic-sync/approval_request_bugfix_frontend.md`.
4. Dile al Humano: *"He dejado mi solicitud en `.agentic-sync/approval_request_bugfix_frontend.md`. Por favor, ve al chat del Bug-Fix Lead y entrégale el mensaje."*
5. Espera el veredicto. Si aprobado, pasa a `EXECUTION`, corrige, haz build y `git commit` + `git push`.
