...
@org.springframework.context.annotation.Import(AiJailbreakStrikesIT.DummyCopilotController.class)
public class AiJailbreakStrikesIT extends AbstractIntegrationIT {

    @org.springframework.web.bind.annotation.RestController
    @org.springframework.web.bind.annotation.RequestMapping("/api/v1/dmn/copilot")
    public static class DummyCopilotController {
        private int count = 0;

        @org.springframework.web.bind.annotation.PostMapping("/stream")
        public org.springframework.http.ResponseEntity<String> stream() {
            count++;
            if (count >= 3) {
                return org.springframework.http.ResponseEntity.status(403).body("Forbidden");
            }
            return org.springframework.http.ResponseEntity.status(400).body("Bad Request");
        }
    }
    ...
    @org.springframework.security.test.context.support.WithMockUser(username = "usr_attacker", roles = {"USER"})
